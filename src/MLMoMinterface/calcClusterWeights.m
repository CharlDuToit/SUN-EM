...
function  [clusterWeights, pred,unity, clusterWeightsInd, unityNonSingMSE, predNonSingMSE,...
    unityNonSingRelNormPercentError, predNonSingRelNormPercentError] = calcClusterWeights(terms, refVals,nonSingZmnProp, clusterInd,edgeLabels, edgeLengths,clusterMaxEdgeLength, singThresh, addBias, minPercentImprov, useProjectedEdges)
...
%             pred(ind,1) = sum(regressedTerms,2);
    ...
    unity  = sum(terms, 2);
    [predNonSingMSE, predNonSingRelNormPercentError] = calcError(refVals, pred);
    [unityNonSingMSE, unityNonSingRelNormPercentError] = calcError(refVals, unity);
